...
// //       padding: '8px 20px',
// //       borderRadius: '100px',
// //       border: selected ? 'none' : '1.5px solid rgba(124,58,237,0.15)',
// //       background: selected
...
// //       cursor: 'pointer',
// //       transition: 'all 0.2s',
// //       boxShadow: selected ? '0 4px 14px rgba(124,58,237,0.35)' : '0 1px 4px rgba(0,0,0,0.05)',
// //       fontFamily: 'inherit',
// //     }}
// //   >
// //     {category}
...
// //   }[exercise.difficulty] || { bg: 'rgba(107,114,128,0.1)', color: '#6b7280' };

// //   const handleStart = () => {
// //     if (exercise.videoUrl?.trim()) {
// //       navigate('/exercises/player', { state: { videoUrl: exercise.videoUrl, title: exercise.title } });
// //     } else {
// //       alert('No video available for this exercise.');
// //     }
// //   };

// //   const handleEnter = () => gsap.to(ref.current, { y: -6, scale: 1.02, duration: 0.3, ease: 'power2.out' });
// //   const handleLeave = () => gsap.to(ref.current, { y: 0,  scale: 1,    duration: 0.45, ease: 'elastic.out(1,0.5)' });
...
// //         borderRadius: '22px',
// //         overflow: 'hidden',
// //         boxShadow: '0 4px 24px rgba(124,58,237,0.07), 0 1px 0 rgba(255,255,255,0.9) inset',
...
// //           color: '#7c3aed', fontSize: '0.7rem', fontWeight: 700,
...
// //             fontWeight: 700, color: '#0f0722', letterSpacing: '-0.02em', lineHeight: 1.2,
// //           }}>
// //             {exercise.title}
// //           </h3>
// //           <span style={{
...
// //         {/* benefits */}
// //         <p style={{ fontSize: '0.875rem', color: '#6b7280', lineHeight: 1.55, flex: 1 }}>
// //           {exercise.benefits}
// //         </p>
...
// //           }}
// //           onMouseEnter={e => { e.currentTarget.style.transform = 'scale(1.02)'; e.currentTarget.style.boxShadow = '0 6px 22px rgba(124,58,237,0.45)'; }}
...
// //   const filtered = exerciseData.filter(ex => selectedCategory === 'All' || ex.category === selectedCategory);

// //   /* reset card refs when filter changes */
...
// //         }
// //         .ex-sub { font-size: 0.95rem; color: #6b7280; font-weight: 400; line-height: 1.55; }
// //         /* header action */
// //         .ex-header-row {
...
// //           color: #7c3aed; font-size: 0.875rem; font-weight: 600;
...
// //           padding: 60px 24px; border-radius: 20px; text-align: center;
// //         }
// //         .ex-loading { background: white; }
// //         .ex-loading-inner { display: flex; flex-direction: column; align-items: center; gap: 14px; }
// //         .ex-spinner {
...
// //         .ex-loading p { font-size: 0.95rem; color: #7c3aed; font-weight: 600; }
// //         .ex-error {
// //           background: rgba(239,68,68,0.05); border: 1px solid rgba(239,68,68,0.2);
// //           color: #dc2626; font-size: 0.95rem;
// //         }
// //         .ex-empty {
// //           background: white; border: 1px dashed rgba(124,58,237,0.15);
...
// //             {categories.map(cat => (
// //               <FilterPill key={cat} category={cat} selected={selectedCategory === cat} onClick={handleFilter} />
// //             ))}
// //             {!loading && (
...
// // ── Video map: category → local mp4 path ──────────────────────────────────
// // Place your videos in /public/videos/ with these exact names.
// // The exercise card will use localVideoPath if present AND videoUrl is empty/missing.
// const CATEGORY_DEMO_VIDEOS = {
...
//               boxShadow: '0 0 6px #34d399', animation: 'liveblink 2s infinite',
...
//           }}
//           onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-2px) scale(1.04)'; e.currentTarget.style.boxShadow = '0 0 40px rgba(34,197,94,0.7), 0 0 80px rgba(34,197,94,0.3)'; }}
//           onMouseLeave={e => { e.currentTarget.style.transform = 'none'; e.currentTarget.style.boxShadow = '0 0 28px rgba(34,197,94,0.5), 0 0 60px rgba(34,197,94,0.2)'; }}
//         >
...
//   useEffect(() => { if (cardRef) cardRef.current[idx] = ref.current; }, []);
...
//   const handleStart = () => {
//     // prefer backend videoUrl, then fall back to local category video
//     const url = exercise.videoUrl?.trim() || CATEGORY_DEMO_VIDEOS[exercise.category] || '';
//     if (url) {
//       navigate('/exercises/player', { state: { videoUrl: url, title: exercise.title } });
//     } else {
//       alert('No video available for this exercise yet. Add a .mp4 to /public/videos/');
//     }
//   };
...
//   // Check if a local video exists for this category
//   const hasLocalVideo = !!CATEGORY_DEMO_VIDEOS[exercise.category];
...
//         display: 'flex', flexDirection: 'column',
//         cursor: 'default', transition: 'box-shadow 0.3s, background 0.3s',
//       }}
//     >
//       {/* image */}
//       <div style={{ height: 160, overflow: 'hidden', position: 'relative' }}>
//         <img src={exercise.imageSrc} alt={exercise.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
//         <span style={{
//           position: 'absolute', top: 12, left: 12,
//           background: 'rgba(255,255,255,0.92)', backdropFilter: 'blur(6px)',
//           color: '#7c3aed', fontSize: '0.7rem', fontWeight: 700,
...
//         {(exercise.videoUrl?.trim() || hasLocalVideo) && (
//           <span style={{
...
//           <h3 style={{
//             fontFamily: "'Sora', sans-serif", fontSize: '1.05rem',
//             fontWeight: 700, color: 'var(--text-primary)', letterSpacing: '-0.02em', lineHeight: 1.2,
//             transition: 'color 0.3s',
//           }}>
//             {exercise.title}
//           </h3>
//           <span style={{ flexShrink: 0, display: 'flex', alignItems: 'center', gap: 4, fontSize: '0.78rem', color: 'var(--text-muted)', fontWeight: 500 }}>
//             🕒 {exercise.duration} min
//           </span>
//         </div>
...
//   const handleFilter = (cat) => { cardRefs.current = []; setSelectedCategory(cat); };
...
//           padding: 5px 14px; border-radius: 100px;
//           font-size: 0.72rem; font-weight: 600; letter-spacing: 0.05em;
//           text-transform: uppercase; color: var(--accent-violet); margin-bottom: 12px;
...
//           background-clip: text;
//         }
//         .ex-sub { font-size: 0.95rem; color: var(--text-muted); font-weight: 400; line-height: 1.55; transition: color 0.3s; }
//         .ex-header-row {
//           display: flex; align-items: flex-end; justify-content: space-between; flex-wrap: wrap; gap: 12px;
//         }
//         .ex-random-btn {
...
//           color: var(--accent-violet); font-size: 0.875rem; font-weight: 600;
//           cursor: pointer; transition: all 0.2s; font-family: inherit;
//           box-shadow: var(--shadow-card);
//         }
//         .ex-random-btn:hover { background: var(--bg-card-hover); border-color: var(--accent-violet); transform: translateY(-1px); }
//         .ex-divider { height: 1px; background: linear-gradient(90deg, var(--border-primary), transparent); margin: 26px 0; }
//         .ex-filter-row { display: flex; gap: 10px; flex-wrap: wrap; align-items: center; }
//         .ex-grid {
//           display: grid;
//           grid-template-columns: repeat(auto-fill, minmax(300px, 1fr));
...
//         .ex-spinner {
//           width: 40px; height: 40px; border-radius: 50%;
//           border: 3px solid var(--border-primary); border-top-color: #7c3aed;
//           animation: spin 0.8s linear infinite;
//         }
//         @keyframes spin { to { transform: rotate(360deg); } }
//         .ex-loading p { font-size: 0.95rem; color: var(--accent-violet); font-weight: 600; }
//         .ex-error { background: rgba(239,68,68,0.05); border: 1px solid rgba(239,68,68,0.2); color: #dc2626; font-size: 0.95rem; }
//         .ex-empty { background: var(--bg-card); border: 1px dashed var(--border-primary); color: var(--text-muted); font-size: 0.95rem; }
//         .ex-count-badge {
//           display: inline-flex; align-items: center; gap: 6px;
//           padding: 6px 14px; border-radius: 100px;
//           background: rgba(124,58,237,0.07); border: 1px solid rgba(124,58,237,0.12);
...
//           border-radius: 18px; border-left: 3px solid #7c3aed;
//         }
//         .ex-video-hint-title {
...
//         }
//         .ex-video-item code { font-family: monospace; font-size: 0.75rem; color: var(--accent-violet); }
//       `}</style>
...
//                 <p className="ex-sub">Evidence-based practices to support your mental wellbeing.</p>
//               </div>
//               <button className="ex-random-btn" onClick={() => {
//                 const cat = categories[Math.floor(Math.random() * (categories.length - 1)) + 1];
//                 handleFilter(cat);
//               }}>
//                 ✨ Random Category
...
//             ) : (
//               filtered.map((ex, i) => (
//                 <ExerciseCard key={ex._id} exercise={ex} cardRef={cardRefs} idx={i} />
//               ))
//             )}
//           </div>

//           {/* ── Video setup hint (only visible if no videos yet) ── */}
//           <div className="ex-video-hint">
//             <div className="ex-video-hint-title">📹 Add Exercise Videos</div>
//             <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: 12 }}>
//               Place your <code style={{ fontFamily: 'monospace', color: 'var(--accent-violet)' }}>.mp4</code> files in <code style={{ fontFamily: 'monospace', color: 'var(--accent-violet)' }}>/public/videos/</code> with these names:
//             </p>
//             <div className="ex-video-list">
//               {Object.entries(CATEGORY_DEMO_VIDEOS).map(([cat, path]) => (
//                 <div className="ex-video-item" key={cat}>
//                   <span>🎬</span>
//                   <div>
//                     <div style={{ fontWeight: 600, color: 'var(--text-secondary)' }}>{cat}</div>
//                     <code>{path.replace('/videos/', '')}</code>
//                   </div>
//                 </div>
//               ))}
//             </div>
...
/*
  Category-wise fallback mapping.
  If backend videoUrl is missing, the app will use the mapped local file.
  You can change this mapping however you want.
*/
...
const ExerciseCard = ({ exercise, cardRef, idx }) => {
  ...
  const handleStart = () => {
    const url = exercise.videoUrl?.trim() || CATEGORY_VIDEO_MAP[exercise.category] || '';
    if (url) {
      navigate('/exercises/player', { state: { videoUrl: url, title: exercise.title } });
    } else {
  ...
  const hasVideo = !!(exercise.videoUrl?.trim() || CATEGORY_VIDEO_MAP[exercise.category]);
      ...
      <div className="ex-root">
        ...
        <div className="ex-main">
          ...
          <div className="ex-video-hint">
            <div className="ex-video-hint-title">📹 Add Exercise Videos</div>
            <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: 12 }}>
              Place your <code style={{ fontFamily: 'monospace', color: 'var(--accent-violet)' }}>.mp4</code>{' '}
              files in <code style={{ fontFamily: 'monospace', color: 'var(--accent-violet)' }}>/public/videos/</code>:
            </p>
            <div className="ex-video-list">
              {LOCAL_VIDEOS.map((path, index) => (
                <div className="ex-video-item" key={path}>
                  <span>🎬</span>
                  <div>
